...
!isTopLevel && DafnyOptions.O.IronDafny && isAbstract, dc);
	...
	void MethodDecl(DeclModifierData dmod, bool allowConstructor, bool isWithinAbstractModule, string dc, out Method/*!*/ m) {
		...
		while (!(StartOf(11))) {SynErr(187); Get();}
		switch (la.kind) {
		case 65: {
			Get();
...
List<Expression> decreases, ref Attributes decAttrs, ref Attributes modAttrs, string caption, bool performThisDeprecatedCheck) {
		...
		while (!(StartOf(16))) {SynErr(202); Get();}
		DocComment(out dc);
		if (la.kind == 68) {
			Get();
			while (la.kind == 74) {
	...
	void OneStmt(out Statement/*!*/ s) {
		...
		while (!(StartOf(19))) {SynErr(211); Get();}
		switch (la.kind) {
		case 75: {
	...
	void NestedCaseStatement(out NestedMatchCaseStmt/*!*/ c) {
		...
		Expect(35);
		while (!(StartOf(31))) {SynErr(253); Get();}
		while (IsNotEndOfCase()) {
			Stmt(body);
			while (!(StartOf(31))) {SynErr(254); Get();}
		}
